...
async def create_client(client_in: ClientCreate, db: AsyncSession = Depends(get_db)):
    ...
    return success_response(data=client.model_dump(), message="Client created successfully")
...
async def get_clients(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    ...
    return success_response(data=[c.model_dump() for c in clients], message="Clients retrieved successfully")
...
async def get_client(client_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return success_response(data=client.model_dump(), message="Client retrieved successfully")
...
async def update_client(client_id: int, client_in: ClientUpdate, db: AsyncSession = Depends(get_db)):
    ...
    return success_response(data=client.model_dump(), message="Client updated successfully")
