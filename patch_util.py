...
# Replace _calculate_tool_cost_for_milestone with a call to FinancialEngine (or just use it directly)
# Let's replace get_dashboard

pattern_dashboard = re.compile(r'    async def get_dashboard\([\s\S]*?        \)', re.DOTALL)
...
new_dashboard = """    async def get_dashboard(
        ...
        for m in milestones:
            ...
            # Active assigned members count
            assigned_members_count = sum(1 for a in m.assignments if a.is_active)
            if assigned_members_count == 0:
                assigned_members_count = 1  # Fallback to 1 if no specific assignments (meaning whole team)
...
# Also patch get_milestone_utilization
pattern_milestone = re.compile(r'    async def get_milestone_utilization\([\s\S]*?        return MilestoneUtilizationResponse\(milestones=rows, total_count=len\(rows\)\)', re.DOTALL)
        ...
        for m in milestones:
            ...
            if not m_start or not m_end:
                rows.append(MilestoneUtilizationRow(
                    ...
                    status=m.status,
                    completion_percentage=m.completion_percentage or 0.0,
                    budget=round(project_budget * (m.weight_percentage / 100.0), 2),
                ))
                continue
            ...
            schedule_variance = actual_duration - planned_duration if actual_duration > 0 else 0
